...
def tree_policy(state):  # state is always the root state at first
    def expand(node):  # node is a game estate whose we want to generate all children
    ...
    if state.is_terminal():
        ...
        return tree_policy(best_child(state, math.sqrt(2)))  # TODO: change c
...
def simulation_policy(state, color):
    while not state.is_terminal():
        ...
        state.make(move)
    # if the next move is ours, we have lost  TODO: VERIFY THIS
...
def backup(state_key, outcome):
    if state_key:
        ...
        backup(tree[state_key]["parent"], -outcome)  # negamax


def best_child(state, c):
    moves = state.generate()  # generate all moves from current state
    state_count = tree[state.get_key()]["count"]  # save the number of times state was visited for future
    children = []
    ...
    # TODO: might crash if not given enough time and a root children has zero count
    best_node = functools.reduce(lambda a, b: a if a["UCT"] > b["UCT"] else b,
                                 map(lambda x: {"state": x["state"],
                                                "UCT": x["values"]["value"] / x["values"]["count"] +
                                                       c * math.sqrt(2 * math.log(state_count)) /
                                                       x["values"]["count"]}, children))
...
def pick_move(state):
    ...
    best_node = functools.reduce(lambda a, b: a if a["count"] > b["count"] else b,
                                 map(lambda x: {"move": x["move"],
                                                "count": x["values"]["count"]}, children))
    return best_node["move"], best_node["count"]


def mtcs(game, check_abort):
    color = game.get_to_move()
    ...
    while not check_abort.do_abort():
        selected_state = tree_policy(copy.deepcopy(game))
        outcome = simulation_policy(copy.deepcopy(selected_state), color)
        backup(selected_state.get_key(), outcome)
        sims = sims + 1
        print(sims)
    print(sims)
    return pick_move(game)  # only return the move
